...
void main() {
    // Map OpenFL coordinates back to standard Shadertoy pixels
    ...
    vec2 p = fragCoord.xy * dist / iResolution.x; // Fixed .xx mapping to single dimension
    p.x -= iTime/1.1;
    ...
    // Output final color over existing game textures
    vec4 baseTex = flixel_texture2D(bitmap, openfl_TextureCoordv);
    gl_FragColor = vec4(color.x, color.y, color.z, alpha) * baseTex.a;
}
